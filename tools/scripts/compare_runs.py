...
def classify_status(row: Dict[str, str]) -> str:
    ...
    if raw_status in {"FLOW_FAIL", "INCOMPLETE"}:
        return "FLOW_FAIL"
    if not (to_float(row.get("setup_wns_ns")) is not None and to_float(row.get("setup_tns_ns")) is not None):
        return "FLOW_FAIL"
...
def write_summary_csv(path: Path, rows: List[Dict[str, str]]) -> None:
    keys = [
        "_variant", "_run_dir", "_artifact", "_clock_requested", "clock_ns", "clock_ns_reported",
        "setup_wns_ns", "setup_tns_ns", "hold_wns_ns", "hold_tns_ns", "core_area_um2", "die_area_um2",
        "instance_count", "utilization_pct", "wire_length_um", "vias_count", "power_total_W", "power_internal_W",
        "power_switching_W", "power_leakage_W", "drc_errors", "drc_errors_klayout", "drc_errors_magic",
        "lvs_errors", "antenna_violations", "antenna_violating_nets", "antenna_violating_pins", "ir_drop_worst_V",
        "status", "selection_reason", "_openlane_run_present", "_attempt_number", "_github_run_id", "_synth_strategy_override",
    ]
...
def write_summary_md(path: Path, rows: List[Dict[str, str]]) -> None:
    ...
    for idx, row in enumerate(ordered):
        ...
        lines.append(
            f"| {row.get('_variant','').replace('|','/')} | {row.get('_run_dir','').replace('|','/')} | {row.get('clock_ns','')} | {row.get('setup_wns_ns','')} | {row.get('setup_tns_ns','')} | {row.get('core_area_um2','')} | {row.get('power_total_W','')} | {row.get('drc_errors','')} | {row.get('lvs_errors','')} | {row.get('antenna_violations','')} | {row.get('status','')} | {remarks.replace('|','/')} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def package_best_bundle(best_bundle_dir: Path, best: Dict[str, Any]) -> None:
    ...
    for name in ("metrics.csv", "metrics.md", "metrics_raw.json", "run_meta.json", "attempt_started.txt", "attempt_manifest.json", "viewer.html", "index.html", "README.txt"):
        src = base_dir / name
        if src.exists():
...
def badge_html(status: str) -> str:
    ...
    cls = "flow"
    if s == "PASS": cls = "pass"
    elif s == "TIMING_FAIL": cls = "timing"
    elif s == "SIGNOFF_FAIL": cls = "signoff"
    elif s == "SIGNOFF_AND_TIMING_FAIL": cls = "mixed"
...
def external_button(href: str, label: str, secondary: bool = False) -> str:
    cls = "btn secondary" if secondary else "btn"
    return f'<a class="{cls}" href="{html.escape(href)}" target="_blank" rel="noopener noreferrer">{html.escape(label)}</a>'
...
def build_site(
    ...
    def metric_group_html(title: str, items: List[Tuple[str, Any]]) -> str:
        return (
            ...
            f'<div class="table-wrap"><table><tr><th>Field</th><th>Value</th></tr>{kv_rows(items)}</table></div>'
            f'</section>'
        )
    ...
    for row in ordered:
        ...
        for name in ("metrics.csv", "metrics_raw.json", "run_meta.json", "attempt_started.txt", "attempt_manifest.json", "viewer.html"):
            src = base_dir / name
            if src.exists():
        ...
        for name in ("metrics.csv", "metrics_raw.json", "run_meta.json", "attempt_manifest.json", "attempt_started.txt"):
    ...
    for idx, row in enumerate(ordered):
        ...
        gds_page = f'runs/{html.escape(row["_site_slug"])}/{html.escape(row.get("_site_gds",""))}' if row.get("_site_gds") else ""
        ...
        rows_html.append(f'<tr><td>{selected_marker}</td><td><a href="{run_page}"><strong>{html.escape(str(row.get("_variant","")))} / {html.escape(str(row.get("_run_dir","")))}</strong></a></td><td>{value_or_dash(row.get("clock_ns"))}</td><td>{value_or_dash(row.get("setup_wns_ns"))}</td><td>{value_or_dash(row.get("setup_tns_ns"))}</td><td>{value_or_dash(row.get("core_area_um2"))}</td><td>{value_or_dash(row.get("power_total_W"))}</td><td>{value_or_dash(row.get("drc_errors"))}</td><td>{value_or_dash(row.get("lvs_errors"))}</td><td>{value_or_dash(row.get("antenna_violations"))}</td><td>{value_or_dash(row.get("ir_drop_worst_V"))}</td><td>{badge_html(row.get("status",""))}</td><td>{html.escape(str(row.get("selection_reason","")))}</td><td>{gds_html}</td><td>{external_button(TT_GDS_VIEWER_URL, "Viewer", True)}</td></tr>')
